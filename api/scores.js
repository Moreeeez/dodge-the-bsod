...
async function restoreLegacyScoresToModeKeys() {
  const legacyScores = await readAllLegacyScores();
  if (!legacyScores.length) return;

  for (const mode of MODE_KEYS) {
    const existing = parseScoreList(await kvRequest(["GET", `${KEY_PREFIX}${mode}`]));
    const legacyForMode = legacyScores.filter(entry => entry.mode === mode);
    if (legacyForMode.length) await writeScores(mode, [...existing, ...legacyForMode]);
  }
}
...
export default async function handler(req, res) {
  try {
    if (req.method === "GET") {
      ...
      await restoreLegacyScoresToModeKeys();
